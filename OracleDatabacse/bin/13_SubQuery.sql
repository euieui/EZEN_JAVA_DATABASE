...
select deptno from emp where ename = 'SCOTT'; -- 결과는 20
...
-- [연습문제] SCOTT과 동일직 업(job)을 가진 사원의 모든 정보를 출력
select * from emp where job=(select job from emp where ename='SCOTT');

-- [연습문제] SCOTT 과 급여과 동일하거나 더 많이 받는 사원이름과 급여 출력
select ename , sal from emp where sal>=(select sal from emp where ename='SCOTT');

-- [서브 쿼리 & 그룹함수]
-- 전체 사원 평균 급여보다 더많은 급여를 받는 사원의 이름 , 급여 ,job
...
-- # 두번째 방법
select sal from emp where deptno=30; -- 30번 부서의 사원들이 받는 급여 들
...
-- [연습문제]
-- 영업 사원 (job = 'SALESMAN) 들의 최소 급여보다 많이 받는 사원들의 이름과 급여와 직급, 급여를 출력하되 영업사원(SALESMAN)은 출력하지 않ㅅ습니다.
-- # 첫번째 방법
