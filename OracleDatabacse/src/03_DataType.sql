...
-- 2. 실수형 타입
--		- number(6,2)는 소수점을 포함한 총자리수가 6자리이고 소수점 둘째 자리까지 잇는 실수형 값이 설정됩니다
...
--		- 반대로 지정한 크리고받 적은 문자가 저장되려고 하면 실제 저장크기가 저장하려문 문자 만큼 줄어들어서 저장됩니다
--		- 기억장소가 절약되는 장점이 있습니다.

-- 4. 고정형 문자열(최대 2000 BYTE)
--		- char(XX)
--		- char(10) 으로 만들어진 필드에 영문자 5크기 만큼 데이터를 저장하면 실제크기와 상관없이 고정형으로 10byte가 저장됩니다.
...
-- 7. 날짜 데이터
--		- DATE : BC 4712년 999년 12월 31일 연월일시분초 입력 가능합니다
...
--		- CLOB : 문자형 대용량 객체,
--					  고정길이와 가변길이 문자집합 지원(최대크기(4GB-1)*(데이터베이스 블록사이즈)
...
-- 9. 지금 잘 사용되지 않는 자료형
--		- varchar : varchar2 와 같은 형식이지만 용량에서 varchar2가 업그레이드 되었으며, 이때문에 잘 사용하지 않ㅅ습니다.
